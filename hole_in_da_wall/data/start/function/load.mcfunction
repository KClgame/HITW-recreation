...
execute as @a run scoreboard players set @s survived_walls 0
execute as @a run scoreboard players set @s temp_20 0
execute as @a run scoreboard players set @s loop_count 0

function hitwnorth:detect_north_wall
function hitwsouth:detect_south_wall
function hitweast:detect_east_wall
function hitwwest:detect_west_wall

scoreboard objectives add returned dummy
execute as @a run scoreboard players set @s returned 0
scoreboard objectives add return_random dummy
...
# ===== 自选平台系统 =====
scoreboard objectives add platform dummy {"text":"自选平台","color":"red"}
execute as @a unless score @s platform matches 1..15 run scoreboard players set @s platform 1
tellraw @a [{"text":"[Walls Survived] ","color":"gold","bold":true},{"text":"自选平台已就绪！用 ","color":"green"},{"text":"/scoreboard players set @s platform 1~15","color":"yellow","bold":true,"underlined":true,"clickEvent":{"action":"suggest_command","value":"/scoreboard players set @s platform "}},{"text":" 选择出生平台（1~15）","color":"green"}]

tag @a add player

schedule function hitwnorth:check_eliminate 10t replace
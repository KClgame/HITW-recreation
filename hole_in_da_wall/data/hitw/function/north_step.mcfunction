...
execute as @e[tag=piston,tag=north,scores={loop_count=24},limit=1] at @s if score @p temp_20 matches 20 run function hitw:big_celebration
...
# 清理 + 准备下一面墙
execute as @e[tag=piston,tag=north,scores={loop_count=24},limit=1] run schedule function hitw:kill_north_pistons 1t append
execute if entity @e[tag=piston,tag=north,scores={loop_count=..23}] run schedule function hitw:north_button 1t replace